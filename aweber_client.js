...
// Request Aweber credentials for the user
// @param options {optional}  XXX support options.requestPermissions
// @param credentialRequestCompleteCallback {Function} Callback function to call on
//   completion. Takes one argument, credentialToken on success, or Error on
//   error.
Aweber.requestCredential = function (options, credentialRequestCompleteCallback) {
  ...
  var config = ServiceConfiguration.configurations.findOne({service: 'aweber'});
  if (!config) {
  ...
  var callbackUrl = Meteor.absoluteUrl('_oauth/aweber?close&state=' + credentialToken);

  // url to app, enters "step 1" as described in
  // packages/accounts-oauth1-helper/oauth1_server.js
  var url = '/_oauth/aweber/?requestTokenAndRedirect='
